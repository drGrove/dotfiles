...
var background = {
    ...
    getProfiles:function() {          
        ...
        $.ajax({
            url: "https://www.google.com/analytics/web/getBaseData?_u.date00=" + today + "&_u.date01=" + today + "&homeAccountsTable-tableControl.searchTerm=&homeAccountsTable.viewType=FLAT&id=home-page&cid=homeAccountsTable%2CtimestampMessage&hl=en_US&authuser=0",
            data: {
                token: this.token
            },
            type: "POST",
            ...
            success: function (data) {
                ...
                for (var i in data.accounts) {
                    ...
                    background.accounts[data.accounts[i].id]= {
                        "label": data.accounts[i].name
                    };
                    
                    
                    for(var j in data.accounts[i].wprops){
                        for(var k in data.accounts[i].wprops[j].profiles){
                            background.profiles[data.accounts[i].wprops[j].profiles[k].datasetId] = {
                                "label": data.accounts[i].wprops[j].name, 
                                "parent" : data.accounts[i].id
                            };
                        }
                    }
                /*        
