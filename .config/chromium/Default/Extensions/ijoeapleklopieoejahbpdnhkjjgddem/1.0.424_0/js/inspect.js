...
function ASLConnection() {
    ...
    this.receive = function (ev) {
        ...
        try {
            ...
            try {
                ...
                SkyLabController.debug(ex.description + "When trying to decrypt");
        ...
        var messageActionMap = {
            ...
            'pair_ready' : function () {
                try {
                    ...
                    SkyLab._dmmsgversion = parseInt(message.version)
                } catch (ex) {
    ...
    this.secureConnection = function (challenge) {
        try {
            var unpacked = Crypto.util.bytesToHex(this.decrypt(challenge, true));
            var mine = unpacked.substring(0,32);
            var yours = unpacked.substring(32,64);
        } catch (ex) {
    ...
    this.decrypt = function (message, asHex) {
        ...
        encryptedBytes = asHex ? Crypto.util.hexToBytes(message) : message;
    ...
    this._formatPairFirst = function () {
        return JSON.stringify({
            ...
            "options" : {
                "id": this._uuid,
                "name" : "Edge Inspect Chrome Extension",
                "type": "administrator",
...
var StorageManager = new ASLStorageManager();

var Connection = new ASLConnection();

var Inventory = new ASLInventory();

var Resource = new ASLResource();

function ASLSkyLab () {
    this._defaultfollowmode     = 'on';
    ...
    this.setDeviceManagerInfo = function (options) {
        ...
        if (parseInt(SkyLab._dmversion.replace(/\./g, "")) >= 104070) {
...
var SkyLabController = new ASLSkyLabController ();
...
var SkyLabView = new ASLSkyLabView();
